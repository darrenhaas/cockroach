...
// A merge can only occur when the range being subsumed is empty.
func (r *Range) AdminMerge(args *proto.AdminMergeRequest, reply *proto.AdminMergeResponse) {
	...
	// Check to make sure the subsumbed range is indeed empty.
	keyCount, err := engine.GetRangeStat(r.rm.Engine(), subsumedDesc.RaftID, engine.StatKeyCount)
	if err != nil {
		reply.SetGoError(util.Errorf("unable to get range stats: %s", err))
		return
	}
	if keyCount > 0 {
		reply.SetGoError(util.Errorf("Range %d is not empty", subsumedDesc.RaftID))
		return
	}
	...
	if err = r.rm.DB().RunTransaction(txnOpts, func(txn *client.KV) error {
		...
		txn.Prepare(proto.Delete, proto.DeleteArgs(makeRangeKey(subsumedDesc.StartKey)),
			deleteResponse)
